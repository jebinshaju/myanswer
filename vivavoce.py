...
def write_test_to_database(test_name, num_questions, question_inputs, answer_inputs):
...
def teacher_dashboard(user):
    ...
    if submit_button:
        write_test_to_database(test_name, num_questions, questions, answers)
...
def handle_exam(exam_code,user):
    ...
    if test_data:
        ...
        with st.form("answer_form"):
            total_marks = 0 
            for i, question_data in enumerate(li):
  ...
  "text2_details": {{
    ...
    "audio": "itally.wav"
                ...
                try:
                    ...
                    st.write(":smile:")
                db.child("students").child(user['localId']).set({"test_id": exam_code, "marks_obtained": total_marks})
...
def main():
    ...
    if page == "Teacher Login" and "user" not in st.session_state:
        ...
        #           # Go to teacher dashboard
        #     except Exception as e:
        #         test_data = db.child("tests").child("23c17").get().val()

        #         st.sidebar.error(f'{e}, == {test_data["questions"]}')
